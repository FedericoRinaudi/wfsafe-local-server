...
impl ClientService {
    pub fn register_client(clients_map: &mut ClientsMap, client_dto: ClientDTO) -> Result<(), EbpfError> {
        let flow = client_dto.flow.into();
        let keys = client_dto.keys.into();
        clients_map.insert(&flow, &keys)
    }

    pub fn unregister_client(clients_map: &mut ClientsMap, flow_dto: FlowDTO) -> Result<(), EbpfError> {
